...
## 9. Recent Enhancements (2025)

### Route Validation System
The application now includes comprehensive route validation to prevent analysis of incorrect or mismatched data:

**Features:**
- **Automatic validation after train lookup**: When a user enters a train number, the system automatically validates that the CSV data matches the expected route
- **Manual route override**: For special cases (short terminations, partial journeys, festival specials), users can manually specify the route using dropdown menus
- **Smart detection**: Preserves multi-direction CSV handling while detecting wrong routes
- **Immediate feedback**: Color-coded validation status (green/yellow/red) appears before analysis

**Validation Checks:**
1. **Data existence**: Verifies data exists after geofence slicing
2. **Route configuration**: Checks if route is configured in system (warns if not)
3. **Station sequence coverage**: Validates actual stations match expected route
4. **Direction reversal detection**: Blocks analysis if direction is completely reversed
5. **Destination verification**: Ensures destination station exists in data
6. **Starting station verification**: Confirms starting station is present

**Manual Override Section:**
Located below train details in the UI, includes:
- Train Number (optional)
- From Station dropdown (populated from route graph)
- To Station dropdown (populated from route graph)
- Direction selection (UP/DN)
- Apply Route button

**API Endpoints:**
- `GET /stations`: Returns all stations from route graph for dropdown population
- `POST /validate_route`: Validates route selection with detailed error/warning messages

**Error Messages:**
- ❌ **Blocks analysis**: No data found, direction reversed, destination/start station missing
- ⚠️ **Warns but allows**: Route not configured (MPS/PSR unavailable), partial station coverage

### Multiple File Upload (Overnight Journeys)
GPS devices download data in 24-hour chunks (00:00:00 to 23:59:59), creating multiple CSV files for overnight journeys. The system now handles this automatically:

**Features:**
- **Multi-select file input**: Users can select multiple CSV files at once (Ctrl+Click or Cmd+Click)
- **Automatic chronological sorting**: Files are sorted by timestamp from the data (earliest first)
- **Intelligent concatenation**: DataFrames are merged vertically in correct order
- **Column compatibility validation**: Ensures all files have matching columns before merge
- **Backward compatible**: Single file uploads continue to work as before

**How It Works:**
1. User selects multiple files (e.g., Day 1 CSV + Day 2 CSV for train departing 22:00, arriving 02:00)
2. System extracts first timestamp from each file
3. Files are sorted chronologically
4. DataFrames are concatenated vertically
5. Standard processing (repair shifted rows, drop columns) applied to merged data

**UI Feedback:**
- Shows count and names of selected files before upload
- Success message indicates number of files merged and total rows
- Example: "2 CSV files merged and loaded (64,002 total rows, 8 cols)"

**Technical Details:**
- Backend endpoint modified to accept `List[UploadFile]`
- Timestamp extraction supports multiple formats (YYYY-MM-DD, DD/MM/YYYY, etc.)
- Column mismatch errors provide specific details about missing/extra columns

### Dynamic PDF Filenames
PDF exports now use descriptive filenames based on journey details instead of generic timestamps:

**Format:** `DDMMYY-TRAINNUMBER-LPNAME.pdf`

**Examples:**
- Date: 15/11/2025, Train: 12319, LP: Surendra → `151125-12319-surendra.pdf`
- Date: 01/01/2025, Train: 12139, LP: Rajesh Kumar → `010125-12139-rajeshkumar.pdf`

**Implementation:**
- **Date extraction**: Parsed from first timestamp in filtered data (supports multiple formats)
- **LP name sanitization**: Converted to lowercase, special characters removed, only alphanumeric kept
- **Fallback handling**: Uses current date if timestamp parsing fails
- **Server-driven**: Filename generated by backend via `Content-Disposition` header
- **Frontend extraction**: Browser reads filename from response header automatically

**Benefits:**
- Easier file organization and identification
- No manual renaming needed
- Chronological sorting in file systems
- Clear identification of train and LP at a glance
